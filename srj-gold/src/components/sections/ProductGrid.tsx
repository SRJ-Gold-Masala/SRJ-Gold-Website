...
function ProductCard({ p, delay }: { p:Product; delay:number }) {
  ...
  return (
    <div ref={ref} style={{ opacity:inView?1:0, transform:inView?"none":"translateY(28px)", transition:`opacity 0.65s ease ${delay}ms, transform 0.65s ease ${delay}ms` }}>
      <Link href={`/products/${p.slug}`} style={{ textDecoration:"none", display:"block", border:"0.5px solid #E8E0D0", borderRadius:10, overflow:"hidden", background:"#fff", transition:"box-shadow 0.25s, transform 0.25s" }}
  ...
  return (
    <>
      <style>{`
        ...
        @media(max-width:768px){ .product-grid-3{ grid-template-columns:1fr 1fr; gap:12px; } }
